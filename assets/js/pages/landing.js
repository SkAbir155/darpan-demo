...
window.renderPage = function () {
  ...
  document.getElementById('hero-collage').innerHTML =
    '<div class="absolute left-0 top-5 h-36 w-[27%] md:h-52 lg:top-8 lg:h-64 -rotate-6 overflow-hidden rounded-2xl ring-2 ring-white/40">' + productImg(left, 'h-full w-full object-cover') + '</div>' +
    '<div class="absolute right-0 top-5 h-36 w-[27%] md:h-52 lg:top-8 lg:h-64 rotate-6 overflow-hidden rounded-2xl ring-2 ring-white/40">' + productImg(right, 'h-full w-full object-cover') + '</div>' +
    '<div class="absolute left-1/2 top-0 h-44 w-[40%] md:h-64 lg:h-80 -translate-x-1/2 overflow-hidden rounded-2xl bg-white ring-4 ring-white shadow-lift">' +
  ...
  document.getElementById('how-steps').innerHTML = steps.map((s, i) =>
    '<li class="flex items-start gap-3 rounded-2xl border border-slate-100 p-3.5 shadow-card lg:flex-col lg:p-5">' +
  ...
  document.getElementById('shop-list').innerHTML = DATA.shops.map((s) => {
    ...
    return '<a href="shop.html?shop=' + s.id + '" class="w-44 shrink-0 rounded-2xl lg:w-auto border border-slate-100 p-3.5 shadow-card">' +
